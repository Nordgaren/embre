...
impl AESCrypter for DefaultAesCrypter {
    ...
    fn aes_compare_slice(
        ...
        let mut temp = [0; 0x100]; // Buffer probably doesn't need to be this size. Will reconsider later.
        let mut total = 0;
        for chunk in plaintext.chunks(block_size) {
            let out = &mut temp[..block_size * 2];
            let written = crypter.update(chunk, out).unwrap(); // .expect("Could not encrypt chunk")
            if written == 0 {
                crypter.finalize(out).unwrap(); // .expect("Could not encrypt chunk")
            }

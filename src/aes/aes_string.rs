...
impl<'a> AESString<'a> {
    ...
    pub fn to_plaintext_string(&self) -> Result<String, FromUtf8Error> {
        String::from_utf8(self.to_plaintext_data().expect("Could not decrypt aes resource for plaintext string."))
    }
    ...
    pub fn to_plaintext_c_string(&self) -> Result<CString, NulError> {
        CString::new(self.to_plaintext_data().expect("Could not decrypt aes resource for plaintext string."))
    }
}
...
impl PartialEq<[u16]> for AESString<'_> {
    fn eq(&self, other: &[u16]) -> bool {
        ...
        aes_u8_cmp(self.crypter.get_cipher(), self.resource, self.key, self.iv, other)
    }
}
impl PartialEq<&[u16]> for AESString<'_> {
    fn eq(&self, other: &&[u16]) -> bool {

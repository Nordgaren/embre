...
pub fn include_str_aes_impl(input: TokenStream) -> TokenStream {
    ...
    let key = if args.key.is_empty() {
        args.key
            .extend(embre_utils::generate_random_bytes(cipher.key_len()));
        &args.key[..]
    } else {
        &args.key[..]
    };
...
pub fn include_bytes_aes_impl(input: TokenStream) -> TokenStream {
...
#[cfg(test)]
mod tests {
    use crate::aes::include_str_aes_impl;
    use quote::quote;
    ...
    fn aes_comparison_operators() {
        ...
        let result = include_str_aes_impl(q.into()).to_string();
